...
class Service implements \FOSSBilling\InjectionAwareInterface
{
     ...
     *
     * @param string $keyword The base domain name to search for
...
@@@METHOD_CONTENT@@@
        ...
        if (isset($options['limit']) && is_numeric($options['limit'])) {
            $suggestions = array_slice($suggestions, 0, (int)$options['limit']);
        }
    ...
    /**
     * Get premium domain availability
     * 
     * @param array $domains Array of domains to check
     * @return array Results with availability and pricing for premium domains
     */
    public function getBulkDomainAvailability(array $domains): array
    {
        $results = [];
        ...
        foreach ($domains as $domain) {
            ...
            $sld = substr($domain, 0, $lastDotPos);
            $tld = substr($domain, $lastDotPos);
            
            $tldModel = $this->tldFindOneByTld($tld);
            if (!$tldModel instanceof \Model_Tld) {
                $results[$domain] = [
                    'domain' => $domain,
                    'available' => false,
            ...
            $available = $this->isDomainAvailable($tldModel, $sld);
            $results[$domain] = [
                'domain' => $domain,
                'available' => $available,
                'price' => $tldModel->price_registration,
                'tld' => $tldModel->tld
            ];
        }
    ...
    /**
     * Get domain WHOIS information for a specific domain
     * 
     * @param string $domain Full domain name to query
    ...
    public function getDomainWhois(string $domain): array
    {
        ...
        $lastDotPos = strrpos($domain, '.');
        if ($lastDotPos === false) {
            throw new \FOSSBilling\InformationException('Invalid domain format');
        }
        ...
        $tldModel = $this->tldFindOneByTld($tld);
        if (!$tldModel instanceof \Model_Tld) {
            throw new \FOSSBilling\InformationException('TLD not supported');
        }
        
        // Get the registrar for this TLD
        $tldRegistrar = $this->di['db']->load('TldRegistrar', $tldModel->tld_registrar_id);
        $adapter = $this->registrarGetRegistrarAdapter($tldRegistrar);
        
        // Create domain object for registrar
        $domainObj = new \Registrar_Domain();
        $domainObj->setTld($tld);
        $domainObj->setSld($sld);
        
        try {
            $whoisDetails = $adapter->getDomainDetails($domainObj);
            
            return [
                'domain' => $domain,
                'registered' => true,
                'registration_date' => $whoisDetails->getRegistrationTime() ? date('Y-m-d H:i:s', $whoisDetails->getRegistrationTime()) : null,
                'expiration_date' => $whoisDetails->getExpirationTime() ? date('Y-m-d H:i:s', $whoisDetails->getExpirationTime()) : null,
                'registrar' => $whoisDetails->getRegistrar() ? $whoisDetails->getRegistrar() : $tldRegistrar->name,
                'status' => $whoisDetails->getStatus(),
                'locked' => $whoisDetails->getLocked(),
                'nameservers' => [
                    'ns1' => $whoisDetails->getNs1(),
                    'ns2' => $whoisDetails->getNs2(),
                    'ns3' => $whoisDetails->getNs3(),
                    'ns4' => $whoisDetails->getNs4()
                ]
            ];
        } catch (\Exception $e) {
    ...
    /**
     * Check if a domain is a premium domain and get its pricing
     * 
     * @param string $domain The domain to check
     * @return array Information about the premium domain
     */
    public function getPremiumDomainPrice(string $domain): array
    {
        // Check if it's a premium domain by checking for special pricing rules
        $lastDotPos = strrpos($domain, '.');
        if ($lastDotPos === false) {
            throw new \FOSSBilling\InformationException('Invalid domain format');
        }
        ...
        // In a full implementation, you would check against a premium domains list 
        // For now, we'll implement basic logic to recognize premium short domains or specific keywords
        
        // Check for premium characteristics: short domains (2-3 chars), dictionary words, common words
        $isPremium = false;
        $premiumMultiplier = 1.0;
        $fixedPremiumPrice = null;
        
        // Short domains (2-3 characters) are often premium
        if (strlen($sld) <= 3 && strlen($sld) >= 2 && ctype_alpha($sld)) {
            $isPremium = true;
            $premiumMultiplier = 10.0; // Set a high multiplier for short domains
        }
        
        // Check for premium keywords - in a real system this would be from a database
        $premiumKeywords = [
            'www', 'web', 'shop', 'buy', 'sell', 'buy', 'sell', 'pay', 'cash', 
            'money', 'bank', 'shop', 'store', 'app', 'api', 'dev', 'vip', 'pro',
            'win', 'top', 'hot', 'fun', 'new', 'net', 'com', 'inc', 'biz'
        ];
        
        if (in_array(strtolower($sld), $premiumKeywords)) {
            $isPremium = true;
            $premiumMultiplier = 5.0;
        }
        
        // Get base price from TLD
        $tldModel = $this->tldFindOneByTld($tld);
        if (!$tldModel instanceof \Model_Tld) {
            throw new \FOSSBilling\InformationException('TLD not supported');
        }
        
        $basePrice = $tldModel->price_registration;
        $premiumPrice = $isPremium ? $basePrice * $premiumMultiplier : $basePrice;
    ...
    /**
     * Search domains with advanced options including premium domains
     * 
     * @param string $query The domain or keyword to search for
     * @param array $options Additional search options
     * @return array Search results with availability and pricing
     */
    public function searchAdvancedDomains(string $query, array $options = []): array
    {
        $results = [];
        
        // If it looks like a full domain, check it directly
        if (strpos($query, '.') !== false) {
            $lastDotPos = strrpos($query, '.');
            $sld = substr($query, 0, $lastDotPos);
            $tld = substr($query, $lastDotPos);
            
            $tldModel = $this->tldFindOneByTld($tld);
            if ($tldModel instanceof \Model_Tld) {
                $available = $this->isDomainAvailable($tldModel, $sld);
                $premiumInfo = $this->getPremiumDomainPrice($query);
            ...
            // If it's just a keyword, generate suggestions with TLDs
            $tlds = $this->di['db']->find('Tld', 'active = 1');
            
            foreach ($tlds as $tld) {
                $domain = $query . $tld->tld;
                $available = $this->isDomainAvailable($tld, $query);
